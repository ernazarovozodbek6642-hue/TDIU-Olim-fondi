...
def generate_ariza_docx(data: dict) -> io.BytesIO:
    ...
    session_name = data.get('session_name') or "Faol sessiya"
    year_run = year_p.add_run(f"{session_name} uchun ariza")
    ...
    _add_field(doc, "Imtiyozingiz borligi (tasdiqlovchi hujjat)", "Yuklangan ✅" if data.get('imtiyozi') else "Yo'q ❌", 18)
    _add_field(doc, "O'qish joyidan ma'lumotnoma (tasdiqlovchi hujjat)", "Yuklangan ✅" if data.get('oqish_joyi_file_id') else "Yo'q ❌", 19)
    ...
    motivation = data.get('motivatsion_xat', '')
    if data.get('motivatsion_file_id'):
        motivation = f"Alohida fayl sifatida biriktirilgan: {data.get('motivatsion_file_name') or 'motivatsion xat'}"
    r_mot = p_mot.add_run(motivation)
