...
class Database:
    ...
    # ─── ADMIN HUQUQLARI ───
    async def create_table_bot_admins(self):
        await self.execute("""
    ...
    async def seed_config_admins(self, admin_ids):
        valid_ids = [str(x) for x in admin_ids if str(x).isdigit()]
        for index, user_id in enumerate(valid_ids):
            await self.execute("""
                INSERT INTO bot_admins (
                    user_id, is_superadmin, can_manage_content,
                    can_manage_documents, can_manage_applications, can_manage_users,
                    is_active, added_by
                ) VALUES ($1, $2, TRUE, TRUE, TRUE, TRUE, TRUE, $1)
                ON CONFLICT (user_id) DO NOTHING
            """, user_id, index == 0, execute=True)

    async def get_admin(self, user_id: str):
        return await self.execute(
            "SELECT * FROM bot_admins WHERE user_id=$1 AND is_active=TRUE",
            str(user_id), fetchrow=True
        )

    async def has_admin_permission(self, user_id: str, permission: str = None) -> bool:
        admin = await self.get_admin(str(user_id))
        if not admin:
            return False
        ...
        column = columns.get(permission)
        return bool(column and admin[column])

    async def get_all_admins(self):
        return await self.execute(
            "SELECT * FROM bot_admins ORDER BY is_superadmin DESC, created_at",
            fetch=True
        )

    async def upsert_admin(self, user_id: str, added_by: str, all_permissions: bool = False):
        return await self.execute("""
            INSERT INTO bot_admins (
                user_id, can_manage_content, can_manage_documents,
                can_manage_applications, can_manage_users, is_active, added_by
            ) VALUES ($1, $3, $3, $3, $3, TRUE, $2)
            ON CONFLICT (user_id) DO UPDATE SET is_active=TRUE
            RETURNING *
        """, str(user_id), str(added_by), all_permissions, fetchrow=True)

    async def set_admin_permission(self, user_id: str, field: str, value: bool):
        columns = {
            'content': 'can_manage_content',
            'documents': 'can_manage_documents',
            'applications': 'can_manage_applications',
        ...
        if not column:
            return None
        return await self.execute(
            f"UPDATE bot_admins SET {column}=$2 WHERE user_id=$1 RETURNING *",
            str(user_id), value, fetchrow=True
        )

    # ─── HUJJAT YUBORISH RUXSATLARI ───
    async def create_table_document_permissions(self):
        await self.execute("""
    ...
    async def set_document_permission(self, user_id: str, session_id: int,
                                      is_allowed: bool, granted_by: str):
        return await self.execute("""
            INSERT INTO document_permissions (user_id, session_id, is_allowed, granted_by)
            VALUES ($1, $2, $3, $4)
            ON CONFLICT (user_id, session_id)
            DO UPDATE SET is_allowed=$3, granted_by=$4, created_at=CURRENT_TIMESTAMP
            RETURNING *
        """, str(user_id), session_id, is_allowed, str(granted_by), fetchrow=True)

    async def has_document_permission(self, user_id: str, session_id: int = None) -> bool:
        if session_id is None:
            return bool(await self.execute(
                "SELECT EXISTS(SELECT 1 FROM document_permissions WHERE user_id=$1 AND is_allowed=TRUE)",
                str(user_id), fetchval=True
            ))
        return bool(await self.execute(
            """SELECT EXISTS(
                   SELECT 1 FROM document_permissions
                   WHERE user_id=$1 AND session_id=$2 AND is_allowed=TRUE
               )""",
            str(user_id), session_id, fetchval=True
        ))

    async def count_document_permitted_users(self):
        return await self.execute(
            "SELECT COUNT(DISTINCT user_id) FROM document_permissions WHERE is_allowed=TRUE",
            fetchval=True
        )

    async def get_user_document_sessions(self, user_id: str):
        return await self.execute("""
            SELECT s.*, p.created_at AS permission_created_at
            FROM document_permissions p
            JOIN sessions s ON s.id=p.session_id
            WHERE p.user_id=$1 AND p.is_allowed=TRUE
    ...
    async def create_table_documents(self):
        ...
        for col, coltype in [
            ...
            try:
                await self.execute(
                    f"ALTER TABLE documents ADD COLUMN IF NOT EXISTS {col} {coltype}",
                    execute=True
                )
            except Exception:
                pass
    ...
    async def add_document(self, user_id: str, theme: str, file_id: str, file_type: str,
                           ...
                           session_id: int = None, batch_id: str = None):
        sql = """
        INSERT INTO documents (
            user_id, theme, file_id, file_type, channel_message_id,
            session_id, batch_id, status, created_at
        )
        VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending', $8)
        RETURNING *;
        """
        return await self.execute(
            sql, user_id, theme, file_id, file_type, channel_message_id,
            session_id, batch_id, created_at, fetchrow=True
        )
    ...
    async def get_document_by_id(self, doc_id: int):
        sql = """
        SELECT d.*, u.real_name, u.full_name, u.language, s.name AS session_name
        FROM documents d
        JOIN users u ON d.user_id = u.id
        LEFT JOIN sessions s ON s.id = d.session_id
        WHERE d.id = $1;
    ...
    async def get_documents_filtered(self, session_id: int, status: str = None,
                                     user_id: str = None, date_from=None, date_to=None):
        conditions = ["d.session_id=$1"]
        args = [session_id]
        if status:
            args.append(status)
        ...
        if user_id:
            args.append(user_id)
            conditions.append(f"d.user_id=${len(args)}")
        if date_from:
            args.append(date_from)
            conditions.append(f"d.created_at >= ${len(args)}")
        if date_to:
            args.append(date_to)
            conditions.append(f"d.created_at < ${len(args)}")
        sql = f"""
    ...
    async def update_document_status(self, doc_id: int, status: str,
        ...
        return await self.execute(
            ...
            doc_id, status, reviewed_by, datetime.datetime.now(), rejection_reason,
            fetchrow=True
        )

    async def search_documents(self, session_id: int, query: str):
        return await self.execute("""
            SELECT d.*, u.real_name, u.full_name, u.language, s.name AS session_name
            FROM documents d
            JOIN users u ON u.id=d.user_id
            LEFT JOIN sessions s ON s.id=d.session_id
            WHERE d.session_id=$1 AND (
                COALESCE(u.real_name, '') ILIKE $2 OR
                COALESCE(u.full_name, '') ILIKE $2 OR
                u.id ILIKE $2 OR
                COALESCE(d.theme, '') ILIKE $2
            )
            ORDER BY d.created_at DESC
        """, session_id, f"%{query}%", fetch=True)
    ...
    async def assign_legacy_records_to_active_session(self):
        active = await self.get_active_session()
        if not active:
            return
        await self.execute(
            "UPDATE arizalar SET session_id=$1 WHERE session_id IS NULL",
            active['id'], execute=True
        )
        await self.execute(
            "UPDATE documents SET session_id=$1 WHERE session_id IS NULL",
            active['id'], execute=True
        )
    ...
    async def create_table_arizalar(self):
        ...
        CREATE TABLE IF NOT EXISTS arizalar (
            ...
            motivatsion_file_id TEXT,
            motivatsion_file_type VARCHAR(20),
            motivatsion_file_name TEXT,
            rejection_reason TEXT,
    ...
    async def add_ariza(self, user_id: str, data: dict, created_at):
        ...
        INSERT INTO arizalar (
            ...
            session_id,
            motivatsion_xat, motivatsion_file_id, motivatsion_file_type,
            motivatsion_file_name, created_at
        ) VALUES (
        ...
        return await self.execute(
            ...
            data.get('session_id'),
            data.get('motivatsion_xat', ''), data.get('motivatsion_file_id'),
            data.get('motivatsion_file_type'), data.get('motivatsion_file_name'), created_at,
            fetchrow=True
        )
    ...
    async def get_ariza_by_id(self, ariza_id: int):
        sql = """
        SELECT a.*, u.full_name, u.username, s.name AS session_name
        FROM arizalar a
        JOIN users u ON a.user_id = u.id
    ...
    async def cancel_ariza(self, user_id: str, ariza_id: int = None):
        if ariza_id is not None:
            sql = "UPDATE arizalar SET status='cancelled' WHERE id=$1 AND user_id=$2 AND status='pending'"
            await self.execute(sql, ariza_id, user_id, execute=True)
            return
