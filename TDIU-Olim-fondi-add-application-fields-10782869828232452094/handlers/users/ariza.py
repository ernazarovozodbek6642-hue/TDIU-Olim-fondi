...
@dp.message_handler(
    lambda m: m.text and m.text.startswith("📬") and (
        "hujjat topshirish" in m.text or "Подача документов" in m.text
    ),
    state='*'
)
async def ariza_start(msg: Message, state: FSMContext):
    ...
    existing = await db.get_ariza_by_user(str(msg.from_user.id), active_session['id'])
    if existing:
    ...
    await _start_ariza_form(msg, lang, active_session)
...
async def ariza_restart(call: CallbackQuery, state: FSMContext):
    ...
    active_session = await db.get_active_session()
    if not active_session:
        await call.message.answer("⛔ Faol sessiya yo‘q." if lang == 'uz' else "⛔ Нет активной сессии.")
        return
    await _start_ariza_form(call.message, lang, active_session)


async def _start_ariza_form(msg: Message, lang: str, active_session=None):
    active_session = active_session or await db.get_active_session()
    if not active_session:
        await msg.answer("⛔ Faol sessiya yo‘q." if lang == 'uz' else "⛔ Нет активной сессии.")
        return
    ...
    if lang == 'uz':
        await msg.answer(
            f"📝 <b>{active_session['name']} uchun ariza</b>\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
        ...
        await msg.answer(
            f"📝 <b>Заявка — {active_session['name']}</b>\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
...
async def cancel_form(msg: Message, state: FSMContext):
    ...
    kb = await (build_main_kb_uz(db, registered_val, user_id) if lang == 'uz' else build_main_kb_ru(db, registered_val, user_id))
    if lang == 'uz':
...
async def user_cancel_ariza(call: CallbackQuery):
    ...
    if ariza['status'] != 'pending':
        ...
        return
    await db.cancel_ariza(str(call.from_user.id), ariza_id)
...
async def get_transkript(msg: Message, state: FSMContext):
    ...
    if msg.text in ["⬅️ Ortga", "⬅️Ortga", "⬅️ Назад", "⬅️Назад"]:
        if lang == 'uz':
            await msg.answer(
                "👫 Aka/uka/opa/singillaringiz haqida ma'lumot kiriting:\n"
                "<i>F.I.Sh. | Ishlash/o'qish joyi | Lavozimi/kursi | Shakli | Shartnoma turi | Tug'ilgan sana</i>\n\n"
...
async def get_oqish_joyi(msg: Message, state: FSMContext):
    ...
    if lang == 'uz':
        await msg.answer(
            ...
            "<i>Javobni bitta xabar matni yoki DOC/DOCX/PDF fayl ko‘rinishida yuboring.</i>",
            reply_markup=ariza_step_kb,
            parse_mode='HTML'
        )
    else:
        await msg.answer(
            ...
            "<i>Отправьте ответ одним сообщением или файлом DOC/DOCX/PDF.</i>",
            reply_markup=ariza_step_kb,
            parse_mode='HTML'
        )
...
@dp.message_handler(state=ArizaStates.motivatsion_xat, content_types=['text', 'document'])
async def get_motivatsion_xat(msg: Message, state: FSMContext):
    ...
    if msg.document:
        filename = msg.document.file_name or 'motivatsion_xat'
        ext = filename.rsplit('.', 1)[-1].lower() if '.' in filename else ''
        if ext not in {'doc', 'docx', 'pdf', 'txt'}:
            await msg.answer(
                "❌ Faqat DOC, DOCX, PDF yoki TXT fayl yuboring."
                if lang == 'uz' else
                "❌ Отправьте файл DOC, DOCX, PDF или TXT."
            )
            return
        await state.update_data(
            motivatsion_xat='',
            motivatsion_file_id=msg.document.file_id,
            motivatsion_file_type='document',
            motivatsion_file_name=filename,
        )
    elif msg.text:
        await state.update_data(
            motivatsion_xat=msg.text.strip(),
            motivatsion_file_id=None,
            motivatsion_file_type=None,
            motivatsion_file_name=None,
        )
    else:
        return
...
@dp.message_handler(state=ArizaStates.confirm, content_types=["text"])
async def confirm_text_handler(msg: Message, state: FSMContext):
    if msg.text == "❌ Arizani to'xtatish":
        await cancel_form(msg, state)
        return
    user = await db.select_user(str(msg.from_user.id))
    lang = user.get('language', 'uz') if user else 'uz'
    ...
    if msg.text in ["⬅️ Ortga", "⬅️Ortga", "⬅️ Назад", "⬅️Назад"]:
        if lang == 'uz':
            await msg.answer(
                ...
                "• O'zingizni haqiqatan munosibman deb o'ylaysizmi?\n"
                "• Nima uchun aynan siz stipendiya g'olibi bo'lishingiz kerak?\n\n"
                "<i>Javobni matn yoki DOC/DOCX/PDF fayl ko‘rinishida yuboring.</i>",
                reply_markup=ariza_step_kb,
                parse_mode='HTML'
            )
        else:
            await msg.answer(
                ...
                "• Считаете ли вы себя достойным кандидатом?\n"
                "• Почему именно вы должны стать стипендиатом?\n\n"
                "<i>Отправьте ответ текстом или файлом DOC/DOCX/PDF.</i>",
                reply_markup=ariza_step_kb,
                parse_mode='HTML'
            )
        await ArizaStates.motivatsion_xat.set()
...
async def ariza_yuborish(call: CallbackQuery, state: FSMContext):
    ...
    if not active_sess or active_sess['id'] != data.get('session_id'):
        await call.message.answer(
            "⛔ Ariza to‘ldirilayotgan paytda faol sessiya o‘zgardi. Iltimos, arizani qaytadan boshlang."
            if lang == 'uz' else
            "⛔ Активная сессия изменилась. Пожалуйста, начните заявку заново."
        )
        return
    ...
    motivatsion_stored_id = None
    if data.get('motivatsion_file_id'):
        motivatsion_stored_id = await upload_to_storage_channel(
            data['motivatsion_file_id'], data.get('motivatsion_file_type', 'document'),
            f"📝 Motivatsion xat | 👤 {fish} (ID: {user_id})"
        )
    ...
    data['motivatsion_file_id'] = motivatsion_stored_id
    ...
    kb = await (build_main_kb_uz(db, True, user_id) if lang == 'uz' else build_main_kb_ru(db, True, user_id))
    ...
    review_admins = [
        ...
        if a['is_active'] and (a['is_superadmin'] or a['can_manage_applications'])
    ]
    for admin in review_admins:
        ...
        try:
            ...
            if motivatsion_stored_id:
                await bot.send_document(
                    admin_id, document=motivatsion_stored_id,
                    caption=f"📝 <b>Motivatsion xat</b> (#{ariza['id']}) — {fish}",
                    parse_mode='HTML'
                )
            elif data.get('motivatsion_xat'):
                await bot.send_message(
                    admin_id,
                    f"📝 <b>Motivatsion xat (#{ariza['id']}):</b>\n\n{data['motivatsion_xat']}",
                    parse_mode='HTML'
                )
