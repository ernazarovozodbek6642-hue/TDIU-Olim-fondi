...
def admin_main_kb(admin):
    is_superadmin = bool(admin['is_superadmin'])
    rows = [[InlineKeyboardButton("📊 Statistika", callback_data="adm:stats")]]
    if is_superadmin or admin['can_manage_users']:
        rows.extend([
    ...
    if is_superadmin or admin['can_manage_documents']:
        rows.append([InlineKeyboardButton("📂 Hujjatlar", callback_data="adm:docs")])
    if is_superadmin or admin['can_manage_applications']:
        rows.append([
            InlineKeyboardButton("📋 Arizalar", callback_data="adm:arizalar"),
            InlineKeyboardButton("📅 Sessiyalar", callback_data="adm:sessions")
        ])
    if is_superadmin or admin['can_manage_content']:
        rows.extend([
    ...
    if is_superadmin:
        rows.append([InlineKeyboardButton("🛡 Admin huquqlari", callback_data="adm:admins")])
...
# ─── ARIZALAR ───
def arizalar_menu_kb(session_id=None):
    suffix = f":{session_id}" if session_id else ""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton("⏳ Kutilayotganlar", callback_data=f"ariza_list:pending{suffix}")],
        [InlineKeyboardButton("✅ Tasdiqlangan", callback_data=f"ariza_list:approved{suffix}")],
        [InlineKeyboardButton("❌ Rad etilgan", callback_data=f"ariza_list:rejected{suffix}")],
        [InlineKeyboardButton("🔍 Qidirish", callback_data=f"ariza_search{suffix}")],
        [InlineKeyboardButton("⬅️ Sessiyalar", callback_data="adm:arizalar")]
    ])


def arizalar_list_kb(arizalar, status, page=0, per_page=8, session_id=None):
    buttons = []
    start = page * per_page
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️", callback_data=f"ariza_page:{status}:{page-1}:{session_id or 0}"))
    ...
    if nav:
        buttons.append(nav)
    buttons.append([InlineKeyboardButton(
        "⬅️ Ortga", callback_data=f"ariza_sess:{session_id}" if session_id else "adm:arizalar"
    )])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
