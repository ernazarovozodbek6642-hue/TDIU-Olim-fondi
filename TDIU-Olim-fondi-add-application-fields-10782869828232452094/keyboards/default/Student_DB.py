...
async def build_main_kb_uz(db, registered: bool = False, user_id: str = None) -> ReplyKeyboardMarkup:
    """
    Asosiy UZ menyu — svc:* bo'limlari aktiv/nofaol holatiga qarab quriladi.
    """
    show_hujjat   = (
        await db.is_service_active('svc:hujjat')
        and bool(user_id)
        and await db.has_document_permission(str(user_id))
    )
    show_ariza    = await db.is_service_active('svc:ariza')
    show_murojaat = await db.is_service_active('svc:murojaat')
    active_session = await db.get_active_session() if show_ariza else None
    ...
    if show_ariza and active_session:
        rows.append([KeyboardButton(
            text=f"📬 {active_session['name']} uchun\nhujjat topshirish"
        )])
...
async def build_main_kb_ru(db, registered: bool = False, user_id: str = None) -> ReplyKeyboardMarkup:
    """
